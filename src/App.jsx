...
export default function App() {
  ...
  async function callGemini(prompt, pdf = null) {
    const body = { prompt };
    if (pdf) body.pdfBase64 = pdf;
  ...
  async function runAudit() {
    ...
    try {
      const context = tab === "url"
        ...
        : `The user has uploaded their actual LinkedIn profile as a PDF. Read every section carefully and base ALL feedback strictly on what is actually in the document. Do not assume or invent anything not present in the PDF.`;

      const pdf = tab === 'pdf' ? pdfBase64 : null;
...
Be brutally specific. No fluff. If a section is missing from the profile, rate it weak.`;
      ...
      const text = await callGemini(prompt, pdf);
